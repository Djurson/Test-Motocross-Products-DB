...
export default function Page() {
  ...
  async function ButtonClickUpload() {
    ...
    setLoading(true);
    await UploadFileToBackend(fileToUpload, category, setStatus, setLoading);
  }
  ...
  return (
    <>
      <div className={`flex flex-col items-center justify-center w-full gap-8 py-6 ${loading ? "h-full" : ""}`}>
        {loading ? (
          <>
            <div className="flex-col gap-4 w-full flex items-center justify-center">
              <div className="w-20 h-20 border-4 border-transparent text-foreground text-4xl animate-spin flex items-center justify-center border-t-foreground rounded-full">
                <div className="w-16 h-16 border-4 border-transparent text-muted-foreground text-2xl animate-spin flex items-center justify-center border-t-muted-foreground rounded-full"></div>
              </div>
            </div>
          </>
        ) : (
...
async function UploadFileToBackend(file: File, rootCategory: string, setStatus: Dispatch<SetStateAction<Status>>, setLoading: Dispatch<SetStateAction<boolean>>) {
  ...
  if (error !== null) {
    setStatus({ error: error, success: null });
  }

  setStatus({ error: null, success: "done" });
}
